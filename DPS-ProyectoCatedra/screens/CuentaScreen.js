...
function CuentaScreen({ navigation }) {
  ...
  const handleLogin = () => {
    const url = 'http://localhost:80/ProyectoCatedra_DPS/api/public/clientes.php?action=login';
    const data = {
      correo: email,
      clave: password
    };
  ...
  const handleLogout = () => {
    const url = 'http://localhost:80/ProyectoCatedra_DPS/api//public/clientes?action=logout';
